...
CommonModel.prototype.winston = require('winston');
...
CommonModel.prototype.connection = function(callback){
    var thisObj = this; 
	//Mongodb = require(this.CONFIG.LIB_PATH + '/class/mongodb.class.js'); 
	thisObj = this;
    ...
    this.mysqlPool.getConnection(function(error, dbConn){
        if(error){
			this.winston.log('info', '连接MYSQL出错'+error.message); 
    ...
    this.connection(function(error, db){
        if(error){
            ...
            db.query(insertSql, values, function(error, results){
                db.end();
                if(error){
					this.winston.log('info', '数据入库失败，插入语句：' + insertSql); 
					/**
                    var mongodbObj = new thisObj.Mongodb();
                    var logJson = {
    ...
    this.connection(function(error, db){
        if(error){
            ...
            db.query(selectSql, preparesValues, function(error, results){
                db.end();
                if(error){
					this.winston.log('info', '数据查询失败，查询语句：' + selectSql); 
					/**
                    var mongodbObj = new thisObj.Mongodb();
                    var logJson = {
    ...
    this.connection(function(error, db){
        if(error){
            ...
            db.query(sql, whereValueArray, function(error, results){
                db.end();
                if(error){
					this.winston.log('info', '数据更新失败，更新语句：' + sql); 
					/**
                    var mongodbObj = new thisObj.Mongodb();
                    var logJson = {
                        'log':'数据更新失败，更新语句：' + sql,
...
CommonModel.prototype.del = function(table, whereCondition, callback){
    this.connection(function(error, db){
        if(error){
            ...
            db.query(sql, whereValueArray, function(error, results){
                if(error){
					this.winston.log('info', '数据删除失败，删除语句：' + sql);
					/**
                    var mongodbObj = new thisObj.Mongodb();
                    var logJson = {
                        'log':'数据删除失败，删除语句：' + sql,
